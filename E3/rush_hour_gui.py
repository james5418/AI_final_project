    ...
    while node_list:
        ...
        for B, car_id, d, s in node.state.expand():
            ...
            if B.board not in visited:            
                g = node.depth + 1
                f = heuristic(B) + g
                tmp = StateNode(f, g, B, node.path+[(car_id, d, s)])
                #tmp = StateNode(f, g, B, node.path+[(B.cars[car_id].car_num, d, s)]); # 紀錄path
...
for play in ans:
    board.play(play[0], play[1], play[2])

    bg = pg.Surface(screen.get_size())
    bg = bg.convert()

    pg.draw.rect(bg,white,[170,70,30,660]) #left wall
    pg.draw.rect(bg,white,[170,700,660,30]) #down wall
    pg.draw.rect(bg,white,[170,70,660,30]) #up wall
    pg.draw.rect(bg,white,[800,70,30,230]) #right up wall
    pg.draw.rect(bg,white,[800,400,30,330]) #right down wall

    for i in range(6):
        for j in range(6):
            pg.draw.rect(bg,color(board.board[i][j]),[200+j*100,100+i*100,100,100])
